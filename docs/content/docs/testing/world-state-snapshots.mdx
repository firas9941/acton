...
<Callout type="note" title="Available since: Acton 1.2">
  These fixtures belong to isolated emulator tests. For persistent simulator and localnet state, use [network snapshots](/docs/environments/how-to/snapshots).
</Callout>
...
get fun `test load snapshot`() {
    ...
    expect(testing.getAccountBalance(target)).toEqual(grams("3"));
}
```
...
get fun `test checkpoint and restore`() {
    ...
    testing.topUp(target, grams("5"));
    expect(testing.saveSnapshot("fixtures/checkpoint.json")).toBeTrue();

    testing.topUp(target, grams("7"));
    expect(testing.getAccountBalance(target)).toEqual(grams("12"));

    expect(testing.loadSnapshot("fixtures/checkpoint.json")).toBeTrue();
    expect(testing.getAccountBalance(target)).toEqual(grams("5"));
}
```
...
- [Gas profiling and snapshots](/docs/testing/gas-profiling-with-snapshots)
