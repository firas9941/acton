...
Acton resolves one masterchain block when the fork starts. Account state and the network configuration come from that block. With an explicit `--fork-block-number`, Unix time also comes from the selected block. A latest fork uses the current system Unix time.

<Callout type="tip" title="Available since: Acton 1.2">
  Fork tests stay in the local emulator. For a persistent fork shared by scripts and frontends, use a [simulator network](/docs/environments/how-to/fork-and-import) in the [Acton Studio](/docs/studio).
</Callout>
...
<Callout type="tip" title="Pin the fork block for faster reruns">
    When in need to rerun the same fork tests often, pass `--fork-block-number` instead of using a live fork. Pinned fork runs persist fetched accounts under `build/cache/<network>/<seqno>/`, so the next CLI run can load the same accounts from disk before calling the remote API. Fork runs without a block number resolve the latest block on every CLI run and reuse persisted accounts only when that block has the same sequence number.
</Callout>

Alternatively, set defaults in `Acton.toml`:
...
Additionally, it is possible to select the custom fork network explicitly for a single run:
...
When fork testing is enabled, Acton first resolves the selected or latest masterchain block and loads the network configuration for the same sequence number. An explicit historical fork also loads that block's time. A latest fork uses the current system time and does not request the block header. When a test touches an account:
...
- Fetched remote snapshots are also saved to disk under `build/cache/<network>/<seqno>/<workchain>_<address-hash>.json`.

Acton separately caches the masterchain configuration and, for explicit historical forks, block time under `build/cache/masterchain-snapshots/<network>/<seqno>.json` for 24 hours. An explicit block number can use a fresh entry without a network request. A fork without a block number first resolves the latest sequence number and then checks for a matching configuration entry.

The shared remote cache stores only fetched remote snapshots:

- The cache key is the tuple of fork network, pinned or resolved block number, and account address.
...
To refetch remote account and library snapshots, run with `--no-fork-cache` once. To also refetch the masterchain snapshot, clear `build/cache`. Changing `--fork-net` or `--fork-block-number` also uses a different cache key.
