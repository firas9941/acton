...
<Callout type="note" title="Available since: Acton 1.2">
  These settings apply to emulator tests, including runs inspected in [Acton Studio](/docs/studio#local-tests). Persistent [simulator networks](/docs/environments/simulator/overview) are configured separately before the workflow continues to testnet and mainnet.
</Callout>
...
Test runs report to [Studio](/docs/studio#local-tests) when it is running for the same
project. Set `[test].studio-reporting = false` or pass `--no-studio-reporting`
to disable reporting. Reporting failures do not change test results.
...
`--no-capture` streams `println` and `eprintln` output while retaining it for
reporters. It does not stream executor logs and cannot be combined with `--mutate`.
...
`gas-profile`, `gas-profile-format`, and `gas-profile-include-tests` are available from Acton 1.2.

Gas profile options apply when `gas-profile` or `--gas-profile <FILE>` is set. Combine `--gas-profile <FILE>` with `--ui` to browse the profile as a flamegraph in Test UI. Add `--gas-profile-include-tests` when the expensive work is executed directly from `.test.tolk` unit-test get methods.
...
`--no-fork-cache` disables persistent account and library cache reads and writes. It does not disable the masterchain snapshot cache. See [fork caching](/docs/testing/fork-testing#cache-scope-and-refresh-behavior).

The fork uses state and network configuration from the same masterchain block. When `fork-block-number` is set, it also uses that block's Unix time; otherwise, it uses the current system Unix time.
