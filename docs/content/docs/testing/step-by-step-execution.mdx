...
<Callout type="tip" title="Available since: Acton 1.2">
  Transaction cursors control isolated emulator tests. To control time in a persistent virtual network, use [simulator time controls](/docs/environments/simulator/time) in the [Acton Studio](/docs/studio).
</Callout>
...
Use [`testing.createExternalTraceIterationCursor()`](/docs/standard_library/emulation/testing#testingcreateexternaltraceiterationcursor)
for an external-in message from `net.createExternalMessage()`:

```tolk
val message = net.createExternalMessage(contractAddress, body);
val iter = testing.createExternalTraceIterationCursor(message);
val first = iter.executeN(1);
val tail = iter.executeAllRemaining();
```

Creating the cursor queues the message without executing it. The cursor uses
no internal sender and retains the external source and optional state init.
If the contract rejects the external message, advancing the cursor fails the test.
This API is available only in emulation.
...
When the generic message type is supplied, Acton fills the opcode search param with that type. Narrow the match further with regular [`SearchParams`](/docs/standard_library/emulation/network#searchparams) filters.
