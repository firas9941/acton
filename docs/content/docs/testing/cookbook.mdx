...
To create a treasury in another workchain, pass it as the second argument:
`testing.treasury("owner", MASTERCHAIN)`. The default is `BASECHAIN`.
...
testing.topUp(passiveAddress, grams("0.5"));
```
