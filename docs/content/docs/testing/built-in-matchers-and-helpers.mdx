...
<Callout type="tip" title="Available since: Acton 1.2">
  Use [Acton Studio](/docs/studio) to review local test traces and inspect emulator test results.
</Callout>
...
The `sendMode` field filters child transactions by the parent send action's mode.
It accepts an exact value or a predicate, like the other `SearchParams` fields.
...
## Numeric and list matchers

<AvailabilityBadge since="Acton 1.2" />

`toBeInRange(min, max)` includes both bounds. `lisp_list<T>` supports
`toContain`, `toNotContain`, `toBeEmpty`, `toBeNonEmpty`, and `toHaveLength`.

`toBeApproxEqRel(expected, maxDelta, maxDeltaDenominator)` accepts a
custom denominator. Its default is `100`; use `1_000_000` for millionths.
See the [matcher reference](/docs/standard_library/testing/expect).

## Get-method execution metadata

<AvailabilityBadge since="Acton 1.2" />

[`net.runGetMethodExt<Ret>()`](/docs/standard_library/emulation/network#netrungetmethodext)
returns the saved stack with `meta.gasUsed` and `meta.exitCode`, including VM failures.
`isSuccess()` accepts exit codes `0` and `1`; `unwrap()` checks success and decodes
as `Ret`. `expect(result).toHaveExitCode(code)` checks the exit code without decoding.
...
- `isAccepted()` reports acceptance during local emulation. After a network broadcast, use `waitForFirstTransaction()` or `waitForTrace()` instead.
...
`acceptanceKnown` indicates whether acceptance is known. After a real-network
broadcast, `isAccepted()` reports an error because submission alone does not
prove acceptance. Use `waitForFirstTransaction()` or `waitForTrace()` to inspect execution.
