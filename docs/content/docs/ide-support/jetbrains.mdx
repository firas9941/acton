...
For installation instructions, see the [official TON documentation](https://docs.ton.org/contracts/ide/jetbrains).
...
For contracts declared in Tolk files, the plugin provides actions to build the contract and generate Tolk or TypeScript wrappers.

![](imgs/jetbrains/contract-actions.png)

### Disassembly

The plugin can compile a Tolk contract and open the generated assembly, making it easier to inspect the code that will run on TVM.
...
Set breakpoints in Tolk tests and contracts, start debugging from the IDE, and inspect stack frames, local values, and TVM registers.
