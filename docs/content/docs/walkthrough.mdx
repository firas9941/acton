...
- [Node.js](https://nodejs.org/en/download/) 22.12 or later LTS: required for the TypeScript app scaffold added by `--app`.
...
<Callout type="idea">
  Pass `--agents` to add coding-agent guidance. See [`acton new`](/docs/commands/new) for all options. When run without arguments, `acton new` prompts interactively.

  <AvailabilityBadge since="Acton 1.2" />

  Pass `--hooks` to install pre-push checks or `--hooks=pre-commit` for checks before each commit.
</Callout>
...
<Callout type="tip" title="Available since: Acton 1.2">
  Test UI's result and transaction inspector, as well as coverage and gas profiling, are included in Acton Studio's <kbd>[Local tests](/docs/studio#local-tests)</kbd>, alongside project-wide test run history.
</Callout>
...
Use [`acton verify`](/docs/commands/verify) to publish Tolk sources that reproduce the contract's compiled code hash. Address checks and verification payments use TON testnet.

Use the same source revision and compiler settings as deployment. First, prepare verification and inspect the payment details:

```bash
acton verify Counter --address <CONTRACT_ADDRESS> --dry-run
```

Replace `<CONTRACT_ADDRESS>` with the testnet address from the [deployment step](#run-scripts).

Acton compiles the contract and requests a verification ticket. If the code hash is already verified, Acton prints its verifier link and exits without payment.

For new code, Acton compares the deployed code hash with the local result and stops on a mismatch. It then prepares the sources and displays the ticket's payment amount, recipient address, and comment.

`--dry-run` does not send a payment or upload sources. It does not confirm that the verifier can compile the source bundle.

To complete verification, use the [funded wallet](#set-up-a-wallet) created earlier:

```bash
acton verify Counter --address <CONTRACT_ADDRESS> --wallet deployer
```

Review the amount and recipient, then approve the payment when prompted. The ticket sets the amount. Keep enough testnet GRAM in the wallet for this payment and network fees.

Acton sends the payment, waits for the recipient transaction to finalize, and uploads the sources with that transaction's hash. On success, it prints a verifier link for the code hash.

To approve the payment through a TON Connect testnet wallet, use `--tonconnect`:

```bash
acton verify Counter --address <CONTRACT_ADDRESS> --tonconnect
```

Acton prints a QR code and a `tc://` link. Connect the wallet and approve the payment in the wallet app.

See the [verification guide](/docs/verify) for compiler options and payment retries.

## Manage on-chain libraries

<Callout type="warn">
  Currently, regular accounts cannot publish libraries on mainnet or testnet. Only special masterchain accounts, such as the ones hosting [elector](https://actonscan.com/config?network=mainnet#config-parameter-1) and [config](https://actonscan.com/config?network=mainnet#config-parameter-0) smart contracts, can publish libraries or perform change-library actions.

  Operations such as obtaining library metadata or performing a top-up are allowed for existing libraries.
</Callout>
...
On a network that permits library publication, publish a configured contract as a library:

```acton-cli
acton library publish <CONTRACT_NAME> --duration 365d --wallet deployer --net <NETWORK> --local
```

Acton compiles the contract, estimates the storage payment for the requested duration, sends the funding transaction, and saves the resulting metadata to `libraries.toml`.

After publishing, inspect the library and top it up before storage fees run out. `info` and `topup` use the network saved in the library metadata:
...
If automatic ABI matching is unavailable, pass compiler ABI JSON or a Tolk interface explicitly:

<AvailabilityBadge since="Acton 1.2" />

```bash
acton rpc info <CONTRACT_ADDRESS> --abi build/abi/Counter.json --net testnet
acton rpc call <CONTRACT_ADDRESS> currentCounter --abi build/abi/Counter.json --net testnet
```
...
Develop alongside [Acton Studio](/docs/studio). Use <kbd>Local tests</kbd> for inspecting test runs, a simulator network for application development, and the same UI and network tools on testnet and mainnet:
