...
- `[wrappers.tolk].output-dir = "wrappers"` and `[contracts.Counter.wrappers.tolk].output-dir = "counter-wrappers"` resolve from project root, but `acton wrapper Counter --output-dir wrappers` resolves from the current working directory.
...
- `[localnet]` — Shared settings for the localnet target and local development services.
...
| `wrappers`     | `table`  | No       | Per-contract overrides for the project-level `[wrappers]` settings.     |
...
[contracts.Precompiled.wrappers.tolk]
output-dir = "generated/precompiled"
generate-test = true
test-output-dir = "tests/generated/precompiled"

[contracts.Precompiled.wrappers.typescript]
output-dir = "app/src/wrappers/precompiled"
```

<AvailabilityBadge since="Acton 1.2" />

The nested `wrappers.tolk` and `wrappers.typescript` tables accept the same
fields as their project-level counterparts. A field set for a contract
overrides only that field; omitted fields continue to use the project-level
value.
...
`output-boc` and `output-sources` are available from Acton 1.2.
...
| `output-boc` | `string` | No | Directory for `<contract-name>.boc` exports, including precompiled contracts. |
| `output-fift` | `string` | No       | Directory where compiled Fift files are written (`<contract>.fif` per `.tolk` contract). |
| `output-sources` | `string` | No    | Directory where source registration artifacts are written (`<contract>.source.json` per `.tolk` contract). |
...
`acton build --out-dir <DIR>`, `acton build --gen-dir <DIR>`, `acton build --output-abi <DIR>`, `acton build --output-boc <DIR>`, `acton build --output-fift <DIR>`, and `acton build --output-sources <DIR>` override these values for a single run. Config values here are project-root-relative. The CLI flags stay relative to the current working directory unless absolute paths are passed. Precompiled `.boc` contracts emit ABI only when their contract config sets `types`; they never emit Fift or source registration output.
...
Per-contract wrapper overrides are available from Acton 1.2.

Override rules:

- For Tolk wrappers, `--output` has highest priority, then `--output-dir`, then `[contracts.<name>.wrappers.tolk].output-dir`, then `[wrappers.tolk].output-dir`, then the `@wrappers` mapping when present, then the wrapper command's built-in `wrappers/` location.
- For TypeScript wrappers, `--output` has highest priority, then `--output-dir`, then `[contracts.<name>.wrappers.typescript].output-dir`, then `[wrappers.typescript].output-dir`, then the wrapper command's built-in default location.
- For Tolk test stubs, `--test-output` has highest priority, then `--test-output-dir`, then `[contracts.<name>.wrappers.tolk].test-output-dir`, then `[wrappers.tolk].test-output-dir`, then the wrapper command's built-in default test location.
- Per-contract `[contracts.<name>.wrappers.tolk].generate-test` overrides `[wrappers.tolk].generate-test`. Both affect only non-TypeScript wrapper generation. `acton wrapper --ts` still cannot be combined with test-stub generation.
...
Configures Simulator defaults and the shared localnet target. `acton simulator` reads
this table. Docker-based `acton localnet` shares the startup `accounts` selection;
its node, timing, storage, and API settings use its own command options.

| Field               | Type       | Default | Description                                                                                                                 |
|:--------------------|:-----------|:--------|:----------------------------------------------------------------------------------------------------------------------------|
| `port`              | `integer`  | `5411`  | HTTP port used by localnet services and tools.                                                                                |
| `db-path`           | `string`   | -       | SQLite database path for persistent node state. Relative paths resolve from project root. |
| `fork-net`          | `string`   | -       | Remote network used as the source of forked localnet state.                                                                 |
| `fork-block-number` | `integer`  | -       | Remote masterchain block sequence number used for a historical localnet fork.                                              |
| `accounts`          | `string[]` | -       | Project wallet names to initialize on `acton simulator start` or `acton localnet start`.                                             |
...
| `block-time-ms` | `integer`  | `500`   | Block production interval in milliseconds when automatic mining is enabled.                                                 |
| `mine-empty-blocks` | `boolean` | `false` | Produce empty blocks during automatic mining. |
| `no-mining`         | `boolean`  | `false` | Disable automatic block production; mine blocks manually with `acton simulator mine` or `/acton_mine`.                       |

<AvailabilityBadge since="Acton 1.2" />

`response-delay-ms`, `block-time-ms`, `no-mining`, and `mine-empty-blocks` are available from Acton 1.2.

The localnet fork uses the network configuration from the resolved masterchain
block. When `fork-block-number` is set, it also uses that block's Unix time;
...
`acton simulator start --port <PORT>`, `--db-path <PATH>`, `--fork-net <NETWORK>`, `--fork-block-number <SEQNO>`, `--accounts <NAME[,NAME...]>`, `--rate-limit <RPS>`, `--response-delay-ms <MS>`, `--block-time-ms <MS>`, `--no-mining`, and `--mine-empty-blocks` override these values for a single run. If `[localnet].no-mining = true`, there is no CLI flag that turns automatic mining back on for that run.
...
| `studio-reporting`          | `boolean`  | `true`        | Send test run data to Acton Studio when it is available.               |
...
`studio-reporting`, `gas-profile`, `gas-profile-format`, and `gas-profile-include-tests` are available from Acton 1.2.

The test fork uses state and network configuration from the same masterchain
block. When `fork-block-number` is set, it also uses that block's Unix time;
otherwise, it uses the current system Unix time.
...
Wallet V5 derives its wallet ID from the network's TON `global-id`. Custom
networks default to `-3`, matching testnet and localnet; set `global-id = -239`
for a custom endpoint that is an alias or proxy for mainnet. Localnet always
keeps `-3`, even when `[localnet].fork-net = "mainnet"`: the fork only provides
remote state and does not turn the local chain into mainnet.

| Field      | Type     | Required                                | Description                                                                                                                                                                                                                                                                                     |
|:-----------|:---------|:----------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `global-id` | `integer` | No                                    | TON network global ID used to derive Wallet V5 IDs. Defaults to `-3` for custom networks. Use `-239` for a mainnet-compatible endpoint. This field is ignored for built-in `mainnet`, `testnet`, and `localnet`. |
...
| `explorer` | `string` | No                                      | Explicit base explorer URL for transaction links. Acton appends `/tx/<hash>` automatically (adds `/tx/` if missing). This network-specific value takes precedence over `--explorer`. For default `localnet`, the derived link is `http://127.0.0.1:<port>/explorer/tx/<hash>`. |

When `explorer` is omitted for `custom:<name>` and both `api.v2` and `api.v3`
are configured, the default Actonscan link carries the network name and API
endpoints as shared-network query parameters. If `api.v3` is unavailable,
Acton keeps the legacy fallback derived from `api.v2`:
