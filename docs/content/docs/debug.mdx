...
<Callout type="tip" title="Available since: Acton 1.2">
  [Acton Studio](/docs/studio) brings local test inspection and network debugging into one workspace. Reproduce behavior in local tests, then a [simulator network](/docs/environments/simulator/overview), before validating on testnet and mainnet.
</Callout>
...
Acton also supports step-by-step transaction-chain control through [`testing.createTraceIterationCursor()`](/docs/standard_library/emulation/testing#testingcreatetraceiterationcursor) and [`TxCursor`](/docs/standard_library/emulation/testing#txcursor). This is separate from the DAP debugger: it controls when later hops in a transaction chain execute.
...
- retrace supports mainnet and testnet; localnet and custom backends are also supported from Acton 1.2
...
Tick-tock replay and `--net localnet` / `--net custom:<name>` are available in 1.2.
Local and custom targets use the TON Center endpoints from `Acton.toml`.
...
- fork state and network configuration come from the same masterchain block; an explicit historical fork also uses that block's Unix time
...
- `--no-fork-cache` disables persistent account and library caching, but it does not disable the 24-hour masterchain snapshot cache
- randomness still comes from the local run model
...
Pass `acton test --no-capture` to stream `println(...)` and `eprintln(...)`
while retaining their output for reporters and Test UI. This flag does not
stream VM debug logs and cannot be combined with `--mutate`.
