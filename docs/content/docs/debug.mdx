...
- pinned fork runs with `--fork-block-number` persist fetched accounts under
  `build/cache/<network>/<seqno>/`, so repeated CLI runs can skip remote account
  fetches for the same network, block, and address
