...
In local emulation, `ExternalSendResult.isAccepted()` reports whether the
contract accepted the external-in message. After a network broadcast, use
`waitForFirstTransaction()` or `waitForTrace()` instead: successful submission
does not prove contract acceptance, so `isAccepted()` exits with an error.
...
See [wrapper upgrade steps](/docs/upgrade-to-1-2#regenerate-wrappers).
...
Override any of these fields for one contract by nesting the same tables under
its contract entry:
...
Acton resolves each field independently, so omitted per-contract fields inherit
the project defaults. CLI output and test options still have the highest
priority. `acton wrapper --all` applies the resulting settings separately to
each contract.
