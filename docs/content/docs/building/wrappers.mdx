...
<Callout type="tip" title="Available since: Acton 1.2">
  Reuse generated wrappers through the [Studio workflow](/docs/studio): local emulator tests, scripts connected to a [simulator network](/docs/environments/how-to/connect), then testnet and mainnet. The `--net` option selects the destination network.
</Callout>
...
Acton reads deployable code from the BoC and reads the ABI from `Precompiled.types.tolk`. A BoC contract without `types` is skipped by `acton wrapper --all`, and `acton wrapper Precompiled` reports that `types` is required. See [Precompiled BoC contracts](/docs/building/precompiled-boc) for the build and wrapper workflow.
...
In local emulation, `ExternalSendResult.isAccepted()` reports whether the contract accepted the external-in message.

<AvailabilityBadge since="Acton 1.2" />

After a network broadcast, use `waitForFirstTransaction()` or `waitForTrace()` instead: successful submission does not prove contract acceptance, so `isAccepted()` exits with an error.
...
### Storage aliases and workchains

<AvailabilityBadge since="Acton 1.2" />

Tolk wrappers accept union aliases as storage types. Generated test stubs use the
corresponding storage types and imports.

`fromStorage(storage, toShard, workchain)` accepts an optional workchain after
`toShard`, defaulting to `BASECHAIN`. For masterchain deployment, use
`Counter.fromStorage(storage, null, MASTERCHAIN)`. Deployments preserve the selected
workchain and shard settings.

Wrapper names and default filenames use the PascalCase contract ID from `Acton.toml`,
independent of the source filename and `display-name`.
...
get fun `test increase`() {
    ...
    val deployResult = contract.deploy(deployer.address, { value: grams("0.05") });
...
fun main() {
    ...
    val contract = Counter.fromStorage({ id: 0, counter: 0 });
    val result = contract.deploy(deployer.address, { value: grams("0.05") });
...
Override any of these fields for one contract by nesting the same tables under its contract entry:
...
Acton resolves each field independently, so omitted per-contract fields inherit the project defaults. CLI output and test options still have the highest priority. `acton wrapper --all` applies the resulting settings separately to each contract.
