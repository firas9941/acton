...
<Callout type="tip" title="Available since: Acton 1.2">
  [Acton Studio](/docs/studio) rebuilds project artifacts as sources change and uses them for inspection across local tests, [simulator development](/docs/environments/simulator/overview), testnet, and mainnet.
</Callout>
...
Pinned fork runs also store fetched remote accounts in this directory under `build/cache/<network>/<seqno>/<workchain>_<address-hash>.json`. `acton test` and `acton script` reuse those files when the same fork network, block number, and account address are requested again. Fork runs without an explicit block use the same cache after they resolve the latest sequence number.

Fork commands store the network configuration and, for explicit historical forks, block time under `build/cache/masterchain-snapshots/<network>/<seqno>.json`. These entries have a 24-hour TTL. The `--no-fork-cache` option affects account and library entries only. Use `--clear-cache` to remove every cache type.
