...
The same fields are available below each contract's `wrappers` table. Missing
per-contract fields inherit their project-level values. `acton wrapper --all`
resolves the effective settings separately for every contract.
