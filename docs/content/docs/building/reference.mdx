...
<Callout type="tip" title="Available since: Acton 1.2">
  [Acton Studio](/docs/studio) applies these build settings while watching the project. The resulting source and ABI artifacts support the [streamlined workflow](/docs/environments/overview).
</Callout>
...
| `--output-boc <DIR>` | Export `<contract-name>.boc` for compiled and precompiled contracts |
| `--output-sources <DIR>` | Export `<contract>.source.json` with source, ABI, compiler, and debug metadata for `.tolk` contracts |
...
`output-boc` also exports precompiled contracts and preserves each contract's `output` path.
`output-sources` exports source bundles for Explorer import; precompiled contracts do not produce source bundles.
Both settings are available from Acton 1.2.
...
- Tolk wrapper path: `--output` -> `--output-dir` -> `[contracts.<name>.wrappers.tolk].output-dir` -> `[wrappers.tolk].output-dir` -> `@wrappers` mapping -> wrapper command built-in default.
- TypeScript wrapper path: `--output` -> `--output-dir` -> `[contracts.<name>.wrappers.typescript].output-dir` -> `[wrappers.typescript].output-dir` -> wrapper command built-in default.
- Test stub path: `--test-output` -> `--test-output-dir` -> `[contracts.<name>.wrappers.tolk].test-output-dir` -> `[wrappers.tolk].test-output-dir` -> wrapper command built-in default.
- `[contracts.<name>.wrappers.tolk].generate-test` overrides `[wrappers.tolk].generate-test`. Both only affect non-TypeScript runs. `acton wrapper --ts` still rejects test-stub options.

<AvailabilityBadge since="Acton 1.2" />

The same fields are available below each contract's `wrappers` table. Missing per-contract fields inherit their project-level values. `acton wrapper --all` resolves the effective settings separately for every contract.
...
[contracts.MyContract.wrappers.tolk]
output-dir = "generated/my-contract"
```
...
| `wrappers`     | table  | No       | Per-contract `tolk` and `typescript` overrides for the project-level wrapper settings                              |
...
Generated wrapper filenames and type names use the contract ID converted to PascalCase. For `[contracts.my_counter]`, that produces `wrappers/MyCounter.gen.tolk` and `wrappers-ts/MyCounter.gen.ts` with the type `MyCounter`, regardless of `src` or `display-name`.
...
The `types` interface and its ABI and wrapper support are available from Acton 1.1.

This is useful for referencing third-party contracts.

For the user-facing workflow, including interface files and wrapper generation, see [Precompiled BoC contracts](/docs/building/precompiled-boc).
...
See [`PrecompiledContractsConfig`](/docs/standard_library/emulation/config#precompiledcontractsconfig) for the standard library helpers used to inspect or override Param 45 in tests.
