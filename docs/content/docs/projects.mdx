...
`acton new` is interactive when stdin/stdout are terminals. It prompts for project name, template, optional app layout, and whether to configure advanced options such as description, license, Git hooks, and `AGENTS.md` guidance.
When running from scripts or CI, pass `--template` explicitly.

Pass flags to skip prompts:

- `--app` — add a React and Vite frontend; supported by all templates.
- `--hooks` — install a pre-push hook under `.githooks/` and set `core.hooksPath`. Use `--hooks=pre-commit` for checks before each commit.
...
It creates `app/` by default, or the path passed to `--create-dapp`. The target directory must not already exist. This mode does not change `Acton.toml`; use `acton new --app` when you want the full [dApp layout](#dapp-layout) in a new scaffold.
...
To create a project with the default layout, run [`acton new`](/docs/commands/new) without `--app`.
...
For a multi-contract project, nest the same tables below a contract to give it
a different layout while keeping the project defaults for every other contract:
...
Only fields present in the contract table override project defaults. This also
works with `acton wrapper --all`, which resolves settings contract by contract.
