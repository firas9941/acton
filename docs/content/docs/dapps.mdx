...
<Callout type="tip" title="Available since: Acton 1.2">
  Use [Acton Studio](/docs/studio) and simulator networks to [connect the frontend](/docs/environments/how-to/connect) through the generated client and TON Connect settings.
</Callout>
...
<Callout>
    The `counter` template ships with a frontend in `app/src/App.tsx` and helpers in `app/src/lib/counter.ts`, already wired to the generated `wrappers-ts/Counter.gen.ts` wrapper.
</Callout>
...
- `counter.getCurrentCounter()` — calls the `currentCounter` get method and returns a typed value
